...
%-----------------------------General Parameters---------------------------
Rb=10e9;                                % Bit rate [bit/s]
Tb=1/Rb;                                % Bit interval [s]
q0=3.5;                                 % Separation between solitons 
Tq=1/(2*q0*Rb);                         % Soliton width [s]
comp=4;                                 % Number of bit slots
samples=64;                             % Samples per slot
N=comp*samples;                         % Total number of samples
time=Tb/samples*(-N/2:(N/2-1));         % Time vector for all slots
f=1/(comp*Tb)*( [1:N] - N/2 - 1);       % Simulation Bandwidth [Hz]
omega=2*pi*f;                           % Angular frequency 
%--------------------------------------------------------------------------

%-----------------------------Fiber Parameters-----------------------------
k=0.01;                                      % Stepsize constant
beta2 = -1e-27;                              % Parameter for GVD [s^2/m]
gama = 1.52e-3;                              % Nonlinear parameter[1/(W*m)]
attenuation = (0.0*1e-3)/(10*log10(exp(1))); % Attenuation [dB/m]
Ld=Tq^2/abs(beta2);                          % Dispersion length [m]
L = 50*Ld;                                   % Fiber Length [m]
L_step=Ld;                                   % Step for the 3D-plot [m]
P0=abs(beta2)/(gama*Tq^2);                   % Pulse peak power [W]
stepsize = L/ceil(L*gama*P0/k);              % Stepesize in [m]
index=find(stepsize>L);                      % The stepsize should not 
stepsize(index)=L;                           %exceed the fiber length
%--------------------------------------------------------------------------
